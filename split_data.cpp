...
int main(int argc, const char *argv[]) {
  ...
  BinaryData *google_data =
      BinaryData::ReadSparseDataJsonFile(google_data_filename);
  ...
  for (auto example : google_data->data()) {
    ...
    std::cout << "Status " << status << " / " << google_data->data_size()
              << std::endl;
  }
  status = 0;
  ...
  for (auto example : cab_data->data()) {
    ...
    std::cout << "Status " << status << " / " << cab_data->data_size()
              << std::endl;
  }
  std::string google_hm_data_filename =
      std::string(google_data_filename) + "_ok";
  std::string google_non_hm_data_filename =
      std::string(google_data_filename) + "_bad";
  std::string cab_hm_data_filename = std::string(cab_data_filename) + "_ok";
  std::string cab_non_hm_data_filename =
      std::string(cab_data_filename) + "_bad";
