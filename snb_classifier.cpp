...
int main(int argc, const char *argv[]) {
  ...
  for (auto i = 0; i < google_test_data.size(); i += batch_size) {
    ...
    for (auto j = i; j < i + batch_size; ++j) {
      google_ll = google_ll * google_network->EvaluateCompleteInstantiation(
                                  google_test_data[j]);
      cab_ll = cab_ll *
               cab_network->EvaluateCompleteInstantiation(google_test_data[j]);
    }
    if (google_ll > cab_ll) {
      accurate += 1;
    } else {
    ...
    total += 1;
    std::cout << "LL Diff: " << (google_ll / cab_ll).parameter()
              << " Correct : " << accurate << " Wrong Google : " << wrong_google
              << " Wrong Cab : " << wrong_cab << " Total : " << total
              << std::endl;
  }
  ...
  for (auto i = 0; i < cab_test_data.size(); i += batch_size) {
    ...
    for (auto j = i; j < i + batch_size; ++j) {
      google_ll = google_ll * google_network->EvaluateCompleteInstantiation(
                                  cab_test_data[j]);
      cab_ll =
          cab_ll * cab_network->EvaluateCompleteInstantiation(cab_test_data[j]);
    }
    if (google_ll < cab_ll) {
      accurate += 1;
    } else {
  ...
  for (const auto& data_pair : google_testing_data->data()){
    ...
    std::cout << "Processing " << status << " / " <<
  google_testing_data->data_size() << std::endl; Probability google_prob =
  google_network->EvaluateCompleteInstantiation(data_pair.first); Probability
  cab_prob = cab_network->EvaluateCompleteInstantiation(data_pair.first); if
  (google_prob > cab_prob){ correct_prediction += data_pair.second;
    }
    total_prediction += data_pair.second;
    std::cout << "Prediction " << correct_prediction << " / " <<
  total_prediction << " correct." << std::endl;
  }
  status = 0;
  for (const auto& data_pair : cab_testing_data->data()){
    status += data_pair.second;
    std::cout << "Processing " << status << " / " <<
  cab_testing_data->data_size() << std::endl; Probability google_prob =
  google_network->EvaluateCompleteInstantiation(data_pair.first); Probability
  cab_prob = cab_network->EvaluateCompleteInstantiation(data_pair.first); if
  (google_prob < cab_prob){ correct_prediction += data_pair.second;
    }
  ...
  std::cout << "Result : " << correct_prediction  << " / " << total_prediction
  << std::endl;
   */
}
