...
PsddNode *LearnPsdd(PsddNode *root_node, BinaryData *data, PsddManager *manager,
                    PsddParameter alpha) {
  std::vector<PsddNode *> serialized_nodes =
      psdd_node_util::SerializePsddNodes(root_node);
  ...
  for (auto it = serialized_nodes.rbegin(); it != serialized_nodes.rend();
    ...
    if (cur_node->node_type() == LITERAL_NODE_TYPE) {
      auto new_literal_node = manager->GetPsddLiteralNode(
          cur_node->psdd_literal_node()->literal(), cur_node->flag_index());
      ...
      PsddParameter smoothed_partition =
          PsddParameter::CreateFromDecimal(total_data_count) + alpha + alpha;
      PsddParameter smoothed_positive =
          PsddParameter::CreateFromDecimal(positive_data_count) + alpha;
      PsddParameter smoothed_negative =
          PsddParameter::CreateFromDecimal(negative_data_count) + alpha;
      PsddNode *new_top_node = manager->GetPsddTopNode(
          cur_top_node->variable_index(), cur_top_node->flag_index(),
      ...
      for (auto i = 0; i < element_size; ++i) {
        ...
        next_parameters.push_back(
            PsddParameter::CreateFromDecimal(data_counts[i]) + alpha);
        partition = partition + next_parameters.back();
      }
      for (auto &cur_param : next_parameters) {
        cur_param = cur_param / partition;
      }
      PsddNode *new_decn_node = manager->GetConformedPsddDecisionNode(
          next_primes, next_subs, next_parameters, cur_node->flag_index());
