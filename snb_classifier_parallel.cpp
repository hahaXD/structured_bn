...
struct EvalResult {
  EvalResult(uintmax_t g_c, uintmax_t g_t, uintmax_t c_c, uintmax_t c_t)
      : google_correct(g_c), google_total(g_t), cab_correct(c_c),
        cab_total(c_t) {}
...
void SingleThread(Network *google_network, Network *cab_network,
                  ...
                  uintmax_t batch_size, size_t offset, size_t data_size,
                  SafeQueue<EvalResult> *safe_queue) {
  ...
  for (auto i = offset; i < offset + data_size; i += batch_size) {
    ...
    for (auto j = i; j < i + batch_size; ++j) {
      google_ll = google_ll * google_network->EvaluateCompleteInstantiation(
                                  google_testing_data->at(j));
      cab_ll = cab_ll * cab_network->EvaluateCompleteInstantiation(
                            google_testing_data->at(j));
    }
    if (google_ll > cab_ll) {
      google_correct += 1;
    ...
    google_total += 1;
    std::cout << " Google correct : " << google_correct
              << " Google total : " << google_total << std::endl;
  }
  ...
  for (auto i = offset; i < offset + data_size; i += batch_size) {
    ...
    cab_total += 1;
    std::cout << " Cab correct : " << cab_correct
              << " Cab total : " << cab_total << std::endl;
  }
  safe_queue->push(
      new EvalResult(google_correct, google_total, cab_correct, cab_total));
}
...
void NetworkConstructThread(const char *google_training_filename,
  ...
  BinaryData *google_training_data =
      BinaryData::ReadSparseDataJsonFile(google_training_filename);
  BinaryData *cab_training_data =
      BinaryData::ReadSparseDataJsonFile(cab_training_filename);
  Network *google_network = Network::GetNetworkFromSpecFile(sbn_filename);
  google_network->LearnParametersUsingLaplacianSmoothing(
      google_training_data, PsddParameter::CreateFromDecimal(1));
  Network *cab_network = Network::GetNetworkFromSpecFile(sbn_filename);
  cab_network->LearnParametersUsingLaplacianSmoothing(
      cab_training_data, PsddParameter::CreateFromDecimal(1));
...
int main(int argc, const char *argv[]) {
  ...
  for (auto i = 0; i < cores; ++i) {
    works.emplace_back(
        std::thread(NetworkConstructThread, google_training_filename,
                    cab_training_filename, sbn_filename, &network_queue));
  }
  for (auto i = 0; i < cores; ++i) {
    works[i].join();
  }
  ...
  BinaryData *google_testing_data =
      BinaryData::ReadSparseDataJsonFile(google_testing_filename);
  BinaryData *cab_testing_data =
      BinaryData::ReadSparseDataJsonFile(cab_testing_filename);
  ...
  for (auto i = min_batch_size; i <= max_batch_size; ++i) {
    ...
    for (auto j = 0; j < cores; ++j) {
      threads.emplace_back(std::thread(
          SingleThread, google_networks[j], cab_networks[j], &google_test_data,
          &cab_test_data, batch_size, j * data_size, data_size, &result));
    }
    ...
    std::cout << "Batch Size : " << batch_size << ";"
              << "Google Correct : " << google_correct << ";"
              << "Google Total : " << google_total
              << "; Cab Correct : " << cab_correct
              << "; Cab Total : " << cab_total << std::endl;
  }
}
/*
  ...
  BinaryData::ReadSparseDataJsonFile(cab_training_filename);
  //Network *google_network = Network::GetNetworkFromSpecFile(sbn_filename);
  //google_network->LearnParametersUsingLaplacianSmoothing(google_training_data,
  PsddParameter::CreateFromDecimal(1));
  //Network *cab_network = Network::GetNetworkFromSpecFile(sbn_filename);
  //cab_network->LearnParametersUsingLaplacianSmoothing(cab_training_data,
  PsddParameter::CreateFromDecimal(1));
  */
